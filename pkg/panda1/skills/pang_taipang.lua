...
taipang:addEffect(fk.TargetSpecifying, {
  anim_type = "offensive",
   can_trigger = function(self, event, target, player, data)
    ...
    and (player:getMark("taipang1-round") == 0 or player:getMark("taipang2-round") == 0)
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    data:cancelTarget(player)
    local choices = {}
    if player:getMark("taipang1-round") == 0 then
        table.insert(choices, 1, "#pang_taipang1")
    end
    if player:getMark("taipang2-round") == 0 then
...
[":pang_taipang"] = "每轮各限一次，当你使用牌指定自己为目标时，你可以取消之并选择一项：将手牌摸至手牌上限；令你的手牌上限+1。",
