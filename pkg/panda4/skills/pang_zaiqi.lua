...
Fk:loadTranslationTable {
  ["pang_zaiqi"] = "再起",
  [":pang_zaiqi"] = "弃牌阶段结束时，若本回合进入弃牌堆的牌数不小于你的体力值，你可以视为使用一张【南蛮入侵】；当你使用【南蛮入侵】对其他角色造成伤害时，你可以防止此伤害并选择一项：其选择令你或其摸一张牌；你回复1点体力。",
...
zaiqi:addEffect(fk.DamageCaused, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    if choice == "zaiqi_recover" then
      ...
      local choices2 = {"player_draw", "self_draw"}
      local choice2 = room:askToChoice(target, {
      choices = choices2,
      skill_name = zaiqi.name,
      })
